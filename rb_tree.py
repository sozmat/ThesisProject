...
class rb_tree(object):
    ...
    def left_rotate(self, current_node):
        """Performs a left rotation on a subtree. Ensures that if current_node does not have right child, then it raises
        KeyError. Referred to page 328 of CLRS book for rotation functions"""

        if self.root is not None and current_node.right is not self.sentinel:
            y = current_node.right
            current_node.right = y.left
            if y.left != self.sentinel:
                y.left.parent = current_node
            y.parent = current_node.parent
            if current_node.parent is self.sentinel:  # curr_node is the root
                self.root = y
    ...
    def right_rotate(self, current_node):
        """Performs a right rotation on a subtree. Ensures that if current_node does not have left child, then it raises
        KeyError. Referred to page 328 of CLRS book for rotation functions"""

        if self.root is not None and current_node.left is not self.sentinel:
            y = current_node.left
    ...
    def __rb_insert_fixup(self, z):
        ...
        # the tree. Referred to page 330 of CLRS book for this function."""
    ...
    def __rb_delete_fixup(self, x):
        ...
        from the tree. Referred to page 338 of CLRS book for this function."""
